 ...
 * @subpackage itop
 */
class itop_datas extends serveur_datas {
	/**
	 * var privee
	 *
	 * @access private
	...
	/*********************** Creation de l'objet *********************/
	/**
	 * Instancie un objet de type itop_datas.
	 * @codeCoverageIgnore
	 ...
	 * @return itop_datas
	 */
	static function &creer_itop_datas(&$liste_option, $sort_en_erreur = false, $entete = __CLASS__) {
		abstract_log::onDebug_standard ( __METHOD__, 1 );
		$objet = new itop_datas ( $sort_en_erreur, $entete );
	 ...
	 */
	public function retrouve_itop_param() {
	 ...
	 */
	public function valide_presence_itop_data($nom, $protocole='both') {
		$this->onDebug ( __METHOD__, 1 );
	 ...
	 * @throws Exception
	 */
	public function recupere_donnees_itop_serveur($nom = "", $wsdl = "") {
		$this->onDebug ( __METHOD__, 1 );
		if ($nom == "") {
			return $this->onError ( "Il faut un nom de itop pour se connecter.", "", 5103 );
		}
		if ($wsdl == "") {
			return $this->onError ( "Il faut un wsdl de itop pour se connecter.", "", 5104 );
		}
		$serveur_data = $this->valide_presence_itop_data ( $nom, 'soap' );
		if ($serveur_data === false) {
			$this->onWarning ( "Pas de configuration de itop pour se connecter." );
			return false;
		}
