...
class wsclient extends Core\wsclient {
	...
	public function &_initialise(
		...
		$this->setObjetotrsDatas ( $liste_class ["datas"] )
	...
	/**
	 * Prepare l'url de connexion au otrs nomme $nom
	 * @param string $nom
	...
	public function prepare_connexion(
		...
		$liste_data_otrs = $this->getObjetotrsDatas ()
			->valide_presence_data ( $nom );
		if ($liste_data_otrs === false) {
			return $this->onError ( "Aucune definition de otrs pour " . $nom );
		}
		/*if (! isset ( $liste_data_otrs ["username"] )) {
			return $this->onError ( "Il faut un username dans la liste des parametres otrs" );
		}
		if (! isset ( $liste_data_otrs ["password"] )) {
			return $this->onError ( "Il faut un password dans la liste des parametres otrs" );
		}*/
		if (! isset ( $liste_data_otrs ["url"] )) {
			return $this->onError ( "Il faut une url dans la liste des parametres otrs" );
		}
		if (isset ( $liste_data_otrs ["account"] )) {
			$this->setAccount ( $liste_data_otrs ["account"] );
		}
		$this->getGestionConnexionUrl ()
			->retrouve_connexion_params ( $liste_data_otrs )
			->prepare_prepend_url ( $liste_data_otrs ["url"] );
    ...
    /**
     * Sends are prepare_requete_json to the otrs API and returns the response as object.
     *
	 ...
	 * @return datas|null
	 */
	public function &getObjetotrsDatas(): ?datas {
		return $this->datas;
	}
	 ...
	 * @codeCoverageIgnore
	 */
	public function &setObjetotrsDatas(
			&$datas): static {
		$this->datas = $datas;
		return $this;
	}
	...
	static public function help(): array|string {
		...
		$help [__CLASS__] ["text"] [] .= "otrs Wsclient :";
