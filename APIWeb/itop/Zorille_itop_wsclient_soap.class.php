...
/**
 * class wsclient_soap<br> Renvoi des information via un webservice.
 * @package Lib
 * @subpackage itop
 */
class wsclient_soap extends Core\abstract_log {
	/**
	 * var privee
	 * @access private
	 * @var datas
	 ...
	 */
	/**
	 * Instancie un objet de type wsclient_soap. @codeCoverageIgnore
	 ...
	 */
	static function &creer_wsclient_soap(&$liste_option, &$datas, $sort_en_erreur = false, $entete = __CLASS__) {
		Core\abstract_log::onDebug_standard ( __METHOD__, 1 );
		$objet = new wsclient_soap ( $sort_en_erreur, $entete );
	...
	public function prepare_connexion($nom) {
		...
		$this ->getObjetSoap () 
			...
			->recupere_donnees_serveur ( $nom, "itopService" ) ) 
			->connect ();
	 ...
	 * @param Core\stdClass|array $object_src
	 * @param Core\SimpleXMLElement $xml_output
	 * @return wsclient_soap
	 */
	 ...
	 * @param string $key
	 * @param Core\stdClass|array|string $value
	 * @param Core\SimpleXMLElement $xml_output
	 * @return wsclient_soap
	 */
	...
	public function __clone() {
		...
		if (is_object ( $this->datas ))
			$this->datas = clone $this ->getObjetItopdatas ();
	}
