...
class liste_ci extends abstract_log {
	 ...
	 */
	/**
	 * Instancie un objet de type liste_ci. @codeCoverageIgnore
	 ...
	 * @return liste_ci
	 */
	static function &creer_liste_ci (
			&$liste_option, 
			&$webservice_rest, 
			$sort_en_erreur = false, 
			$entete = __CLASS__) {
		Core\abstract_log::onDebug_standard ( __METHOD__, 1 );
		$objet = new liste_ci ( $sort_en_erreur, $entete );
	...
	public function ajoute_ci (
			&$ci) {
		if (! $ci instanceof ci) {
			return $this ->onError ( '$ci doit etre une instance de ci' );
		}
