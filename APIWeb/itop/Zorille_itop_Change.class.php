...
class Change extends ci {
	 ...
	 */
	/**
	 * Instancie un objet de type Change. @codeCoverageIgnore
	 ...
	 * @return Change
	 */
	static function &creer_Change(
			&$liste_option,
			&$webservice_rest,
			$sort_en_erreur = false,
			$entete = __CLASS__) {
		Core\abstract_log::onDebug_standard ( __METHOD__, 1 );
		$objet = new Change ( $sort_en_erreur, $entete );
	...
	public function retrouve_Change(
			$name) {
	 ...
	 */
	public function prepare_params_Change(
			$parametres) {
	 ...
	 * @return Change
	 */
	public function creer_oql_Change(
			$fields = array ()) {
		$filtre = array ();
	 ...
	 * @return Change
	 */
	public function gestion_Change(
			$parametres) {
		$this->onDebug ( __METHOD__, 1 );
		$params = $this->prepare_params_Change ( $parametres );
		$this->onDebug ( $params, 1 );
		return $this->valide_mandatory_fields ()
			->creer_oql_Change ( $parametres )
