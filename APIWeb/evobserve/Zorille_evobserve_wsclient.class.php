...
class wsclient extends Core\wsclient {
	...
	public function &_initialise(
		...
		$this->setObjetevobserveDatas ( $liste_class ["datas"] )
	...
	/**
	 * Prepare l'url de connexion au evobserve nomme $nom
	 * @param string $nom
	...
	public function prepare_connexion(
		...
		$liste_data_evobserve = $this->getObjetevobserveDatas ()
			->valide_presence_data ( $nom );
		if ($liste_data_evobserve === false) {
			return $this->onError ( "Aucune definition de evobserve pour " . $nom );
		}
		if (! isset ( $liste_data_evobserve ["username"] )) {
			return $this->onError ( "Il faut un username dans la liste des parametres evobserve" );
		}
		if (! isset ( $liste_data_evobserve ["password"] )) {
			return $this->onError ( "Il faut un password dans la liste des parametres evobserve" );
		}
		if (! isset ( $liste_data_evobserve ["url"] )) {
			return $this->onError ( "Il faut une url dans la liste des parametres evobserve" );
		}
		$this->getGestionConnexionUrl ()
			->retrouve_connexion_params ( $liste_data_evobserve )
			->prepare_prepend_url ( $liste_data_evobserve ["url"] );
		// On prepare l'objet utilisateurs de la connexion, car l'objet curl ne connait pas l'objet datas
		$this->userLogin ( array (
				'login' => $liste_data_evobserve ["username"],
				'password' => $liste_data_evobserve ["password"]
		) );
		return $this;
	}

	/**
	 * Http evobserve params creator
	 *
	 * @return wsclient
	 */
    ...
    /**
     * Sends are prepare_requete_json to the evobserve API and returns the response as object.
     *
	 ...
	 * @return datas|null
	 */
	public function &getObjetevobserveDatas(): ?datas {
		return $this->datas;
	}
	 ...
	 * @codeCoverageIgnore
	 */
	public function &setObjetevobserveDatas(
			&$datas): static {
		$this->datas = $datas;
		return $this;
	}
	...
	static public function help(): array|string {
		...
		$help [__CLASS__] ["text"] [] .= "evobserve Wsclient :";
