...
class ModeleHosts extends globalapi {
	...
	public function retrouve_check_template(): static {
		...
		$this->onDebug ( $donnee_evobserve, 2 );
		// Si il n'y a qu'une donnee, on recree le tableau
		if (isset ( $donnee_evobserve ["nom"] ) && isset ( $donnee_evobserve ["id"] )) {
			$donnee_evobserve = array (
					"0" => $donnee_evobserve
			);
		}
		foreach ( $donnee_evobserve as $modeleHost ) {
