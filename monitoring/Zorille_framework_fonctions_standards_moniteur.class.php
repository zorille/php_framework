...
/**
 * class fonctions_standards_moniteur<br> Gere un point de monitoring.
 *
 * @package Lib
...
class fonctions_standards_moniteur extends abstract_log {
	 ...
	 */
	/**
	 * Parse une ligne de log PHP.<br> Il met a jour un objet moniteur.
	 *
	...
	/**
	 * Parse un fichier de log PHP obligatoire.<br> Il met a jour un objet moniteur.
	 *
	 ...
	 */
	/**
	 * Verifie la presence d'un ou plusieurs processus.<br> Il affiche la liste de ces processus dans le moniteur.
	 *
	 * @param string $nom_processus Processus a retrouver.
	 ...
	 * @return array
	 */
	/* public function valide_jobs(&$resultat_runtime_slurm, $type_traitement) { $liste_erreurs = array (); // On valide que tous les traitements sont ok foreach ( $resultat_runtime_slurm as $slurmJob ) { if ($slurmJob ["type"] != $type_traitement) { // Au cas ou continue; } switch ($slurmJob ["etat"]) { case "ok" : continue; case "corriger" : case "annuler" : continue; case "en cours" : // Valide l'heure d'attribution if ($this->horaire->activeAlarme ()) { $liste_erreurs [( string ) $slurmJob ["_id"]] ["job"] = $slurmJob; $liste_erreurs [( string ) $slurmJob ["_id"]] ["erreur"] = "Le Job : " . ( string ) $slurmJob ["_id"] . " n'est pas termin&eacute; apr&eacute;s " . $this->horaire->getHoraireDebutMax (); } break; case "distribution" : // Valide l'heure de distribution if ($this->horaire->activeAlarme ()) { $liste_erreurs [( string ) $slurmJob ["_id"]] ["job"] = $slurmJob; $liste_erreurs [( string ) $slurmJob ["_id"]] ["erreur"] = "Le Job : " . ( string ) $slurmJob ["_id"] . " n'est pas attribu&eacute; par slurm apr&eacute;s " . $this->horaire->getHoraireDebutMax (); } break; default : // Tout autre etat que les precedents $liste_erreurs [( string ) $slurmJob ["_id"]] ["job"] = $slurmJob; $liste_erreurs [( string ) $slurmJob ["_id"]] ["erreur"] = "Le job " . ( string ) $slurmJob ["_id"] . " (" . $slurmJob ["collection"] . ") est en " . $slurmJob ["etat"] . " dans slurm &agrave; la date : " . date ( "d-m-Y H:i:s", $slurmJob ["date_fin"]->sec ); } } return $liste_erreurs; } */
	/**
