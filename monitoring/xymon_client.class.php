 ...
 */
class xymon_client extends moniteur {
	/**
	 * var privee
	 *
	 * @access private
	 ...
	 * @var string
	 */
	private $bb_client_cmd = "/home/hobbit/server/bin/bb";
	/**
	 * var privee
	 *
	 * @access private
	...
	/*********************** Creation de l'objet *********************/
	/**
	 * Instancie un objet de type xymon_client.
	 * @codeCoverageIgnore
	 * @param options $liste_option Reference sur un objet options
	 * @return xymon_client false un objet MONITEUR ou FALSE en cas d'erreur.
	 * @param string $sort_en_erreur Prend les valeurs oui/non
	 * @param string $entete Entete des logs de l'objet
	 * @return xymon_client
	 */
	static function &creer_xymon_client(&$liste_option, $xymon_client_titre = "", $entete = __CLASS__) {
		$xymon_client = new xymon_client ( $liste_option->verifie_parametre_standard ( "xymon_client[@sort_en_erreur='oui']" ), $entete );
		$xymon_client->_initialise ( array (
				"options" => $liste_option,
				"titre" => $xymon_client_titre 
		) );
	 ...
	 * @return moniteur
	 */
	public function retrouve_xymon_client_param($xymon_client_titre) {
		...
		if ($this->getListeOptions ()
			->verifie_variable_standard ( array (
				"xymon_client",
				"titre" 
		) ) !== false && $xymon_client_titre == "") {
			$xymon_client_titre = $this->getListeOptions ()
				->renvoi_variables_standard ( array (
					"xymon_client",
					"titre" 
			) );
		}
		$this->ecrit ( "<div align=center><font>" . $xymon_client_titre . "</font></div>\n\n" );
	 ...
	 * Renvoi la couleur en cours de la page du xymon_client
	 * (pas d'une ligne).
	 *
	 ...
	 * Ajoute du texte au xymon_client en cours.<br>
	 * On peut ajouter une couleur specifique pour les donnees ajoutees
	 * grace a $ajoute_couleur
	 *
	 * @param string $donnees Texte a ajouter dans le xymon_client.
	...
	/**
	 * Applique la ligne de commande a travers une connexion SSH vers le serveur xymon
	 * @param string $CMD
	 * @return xymon_client
	 */
	...
	static function help() {
		...
		$help [__CLASS__] ["text"] [] .= "Gestion d'un xymon_client";
		$help [__CLASS__] ["text"] [] .= "\t--xymon_client_bin /usr/bin/bb  chemin de la commande bb on bbwin";
		$help [__CLASS__] ["text"] [] .= "\t--xymon_client_status 30              Temps en minutes";
		$help [__CLASS__] ["text"] [] .= "\t--xymon_client_type_os linux/win      defini l'os utilise par client de monitoring";
		$help [__CLASS__] ["text"] [] .= "\t--xymon_client_titre \"titre\"        Titre affiche dans le xymon_client";
