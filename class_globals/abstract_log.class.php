...
/**
 * class abstract_log<br>
 * Gere les fonction standard pour afficher les logs.
 * 
 * @package Lib
...
abstract class abstract_log {
	 ...
	 */
	function __construct($sort_en_erreur = false, $nom_module = "abstract_log") {
	 ...
	 * @return object $this
	 */
	public function onDebug($message, $niveau) {
	 ...
	 * @throws Exception
	 */
	public function onError($message, $donnee_sup = "", $code_retour = 1) {
		if (! is_array ( $message ) && ! is_object ( $message )) {
			$message = "(" . $this ->getEntete () . ") " . $message;
		}
		if ($code_retour == 0) {
	 ...
	 */
	static public function onError_standard($ligne, $tableau_output = "", $code_retour = 1, $verbose = true) {
		if (abstract_log::$logs instanceof logs) {
	 ...
	 * @return true Renvoi TRUE.
	 */
	static public function onWarning_standard($ligne, $verbose = true) {
	 ...
	 * @return true Renvoi TRUE.
	 */
	static public function onInfo_standard($ligne, $verbose = false) {
	 ...
	 * @return true Renvoi TRUE.
	 */
	static public function onDebug_standard($ligne, $niveau = 2, $verbose = false) {
	 ...
	 */
	static function verifie_niveau_verbose($niveau) {
		$retour = false;
		if (abstract_log::$logs instanceof logs) {
	 ...
	 */
	static function prepare_entete($type) {
	 ...
	 */
	static public function affiche_ligne($ligne, $niveau = 0, $verbose = false) {
	 ...
	 * @return bool true si affiche,false sinon.
	 */
	static public function affiche_tableau($tableau, $entete, $niveau = 0, $verbose = false) {
	 ...
	 * @return bool true si affiche,false sinon.
	 */
	static public function affiche_object($class, $entete, $niveau = 0, $verbose = false) {
	 ...
	 * @return string
	 */
	static function colorize($text, $couleur) {
	 ...
	 * @return string
	 */
	static function retrouve_couleur($couleur) {
		switch ($couleur) {
			# Reset
			case 'Color_Off' :
	 ...
	 * @codeCoverageIgnore
	 */
	public function setSortEnErreur($sort_en_erreur) {
	 ...
	 * @codeCoverageIgnore
	 */
	public function setMessage($message_erreur) {
	 ...
	 * @codeCoverageIgnore
	 */
	public function &setListeOptions(&$ListeOptions) {
